...
class TestTeams:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        team = client.ticketing.teams.retrieve(
    ...
    def test_method_list(self, client: Merge) -> None:
        team = client.ticketing.teams.list()
        assert_matches_type(SyncPage[shared.Team], team, path=["response"])
...
class TestAsyncTeams:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        team = await client.ticketing.teams.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(shared.Team, team, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        team = await client.ticketing.teams.list()
        assert_matches_type(AsyncPage[shared.Team], team, path=["response"])
