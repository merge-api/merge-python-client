...
class TestTags:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        tag = client.ticketing.tags.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(shared.Tag, tag, path=["response"])
    ...
    def test_method_list(self, client: Merge) -> None:
        tag = client.ticketing.tags.list()
        assert_matches_type(SyncPage[shared.Tag], tag, path=["response"])
...
class TestAsyncTags:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        tag = await client.ticketing.tags.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(shared.Tag, tag, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        tag = await client.ticketing.tags.list()
        assert_matches_type(AsyncPage[shared.Tag], tag, path=["response"])
