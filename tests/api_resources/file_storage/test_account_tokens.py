...
class TestAccountTokens:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        account_token = client.file_storage.account_tokens.retrieve(
            "string",
        )
        assert_matches_type(shared.AccountToken, account_token, path=["response"])


class TestAsyncAccountTokens:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        account_token = await client.file_storage.account_tokens.retrieve(
            "string",
        )
        assert_matches_type(shared.AccountToken, account_token, path=["response"])
