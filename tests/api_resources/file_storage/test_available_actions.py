...
class TestAvailableActions:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        available_action = client.file_storage.available_actions.retrieve()
        assert_matches_type(shared.AvailableAction, available_action, path=["response"])


class TestAsyncAvailableActions:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        available_action = await client.file_storage.available_actions.retrieve()
        assert_matches_type(shared.AvailableAction, available_action, path=["response"])
