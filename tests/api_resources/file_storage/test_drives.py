...
class TestDrives:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        drive = client.file_storage.drives.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(Drive, drive, path=["response"])
    ...
    def test_method_list(self, client: Merge) -> None:
        drive = client.file_storage.drives.list()
        assert_matches_type(SyncPage[Drive], drive, path=["response"])
...
class TestAsyncDrives:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        drive = await client.file_storage.drives.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(Drive, drive, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        drive = await client.file_storage.drives.list()
        assert_matches_type(AsyncPage[Drive], drive, path=["response"])
