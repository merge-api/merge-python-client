...
class TestUsers:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        user = client.file_storage.users.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(FileStorageUser, user, path=["response"])
    ...
    def test_method_list(self, client: Merge) -> None:
        user = client.file_storage.users.list()
...
class TestAsyncUsers:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        user = await client.file_storage.users.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(FileStorageUser, user, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        user = await client.file_storage.users.list()
