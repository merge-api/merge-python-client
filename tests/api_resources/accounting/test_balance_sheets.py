...
class TestBalanceSheets:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        balance_sheet = client.accounting.balance_sheets.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(BalanceSheet, balance_sheet, path=["response"])
    ...
    def test_method_list(self, client: Merge) -> None:
        balance_sheet = client.accounting.balance_sheets.list()
        assert_matches_type(SyncPage[BalanceSheet], balance_sheet, path=["response"])
...
class TestAsyncBalanceSheets:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        balance_sheet = await client.accounting.balance_sheets.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(BalanceSheet, balance_sheet, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        balance_sheet = await client.accounting.balance_sheets.list()
        assert_matches_type(AsyncPage[BalanceSheet], balance_sheet, path=["response"])
