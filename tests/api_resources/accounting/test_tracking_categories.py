...
class TestTrackingCategories:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        tracking_category = client.accounting.tracking_categories.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(TrackingCategory, tracking_category, path=["response"])
    ...
    def test_method_list(self, client: Merge) -> None:
        tracking_category = client.accounting.tracking_categories.list()
        assert_matches_type(SyncPage[TrackingCategory], tracking_category, path=["response"])
...
class TestAsyncTrackingCategories:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        tracking_category = await client.accounting.tracking_categories.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(TrackingCategory, tracking_category, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        tracking_category = await client.accounting.tracking_categories.list()
        assert_matches_type(AsyncPage[TrackingCategory], tracking_category, path=["response"])
