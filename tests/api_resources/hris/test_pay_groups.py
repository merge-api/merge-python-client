...
class TestPayGroups:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        pay_group = client.hris.pay_groups.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(PayGroup, pay_group, path=["response"])
    ...
    def test_method_list(self, client: Merge) -> None:
        pay_group = client.hris.pay_groups.list()
        assert_matches_type(SyncPage[PayGroup], pay_group, path=["response"])
...
class TestAsyncPayGroups:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        pay_group = await client.hris.pay_groups.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(PayGroup, pay_group, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        pay_group = await client.hris.pay_groups.list()
        assert_matches_type(AsyncPage[PayGroup], pay_group, path=["response"])
