...
class TestCompanies:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        company = client.hris.companies.retrieve(
    ...
    def test_method_list(self, client: Merge) -> None:
        company = client.hris.companies.list()
        assert_matches_type(SyncPage[Company], company, path=["response"])
...
class TestAsyncCompanies:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        company = await client.hris.companies.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(Company, company, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        company = await client.hris.companies.list()
        assert_matches_type(AsyncPage[Company], company, path=["response"])
