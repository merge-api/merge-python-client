...
class TestLocations:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        location = client.hris.locations.retrieve(
    ...
    def test_method_list(self, client: Merge) -> None:
        location = client.hris.locations.list()
        assert_matches_type(SyncPage[Location], location, path=["response"])
...
class TestAsyncLocations:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        location = await client.hris.locations.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(Location, location, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        location = await client.hris.locations.list()
        assert_matches_type(AsyncPage[Location], location, path=["response"])
