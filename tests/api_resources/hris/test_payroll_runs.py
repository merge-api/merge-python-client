...
class TestPayrollRuns:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        payroll_run = client.hris.payroll_runs.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(PayrollRun, payroll_run, path=["response"])
    ...
    def test_method_retrieve_with_all_params(self, client: Merge) -> None:
        payroll_run = client.hris.payroll_runs.retrieve(
            ...
            remote_fields="run_state",
            show_enum_origins="run_state",
        )
        assert_matches_type(PayrollRun, payroll_run, path=["response"])

    @parametrize
    def test_method_list(self, client: Merge) -> None:
        payroll_run = client.hris.payroll_runs.list()
        assert_matches_type(SyncPage[PayrollRun], payroll_run, path=["response"])
...
class TestAsyncPayrollRuns:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        payroll_run = await client.hris.payroll_runs.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(PayrollRun, payroll_run, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        payroll_run = await client.hris.payroll_runs.list()
        assert_matches_type(AsyncPage[PayrollRun], payroll_run, path=["response"])
