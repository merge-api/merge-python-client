...
class TestTasks:
    ...
    def test_method_list(self, client: Merge) -> None:
        task = client.crm.tasks.list()
        assert_matches_type(SyncPage[Task], task, path=["response"])
    ...
    def test_method_list_remote_field_classes(self, client: Merge) -> None:
        task = client.crm.tasks.list_remote_field_classes()
        assert_matches_type(SyncPage[shared.RemoteFieldClass], task, path=["response"])
...
class TestAsyncTasks:
    ...
    async def test_method_create(self, client: AsyncMerge) -> None:
        task = await client.crm.tasks.create(
            model={},
        )
        assert_matches_type(TaskCreateResponse, task, path=["response"])
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        task = await client.crm.tasks.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(Task, task, path=["response"])
    ...
    async def test_method_update(self, client: AsyncMerge) -> None:
        task = await client.crm.tasks.update(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
            model={},
        )
        assert_matches_type(TaskCreateResponse, task, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        task = await client.crm.tasks.list()
        assert_matches_type(AsyncPage[Task], task, path=["response"])
    ...
    async def test_method_list_remote_field_classes(self, client: AsyncMerge) -> None:
        task = await client.crm.tasks.list_remote_field_classes()
        assert_matches_type(AsyncPage[shared.RemoteFieldClass], task, path=["response"])
