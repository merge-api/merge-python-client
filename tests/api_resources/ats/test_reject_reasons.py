...
class TestRejectReasons:
    ...
    def test_method_retrieve(self, client: Merge) -> None:
        reject_reason = client.ats.reject_reasons.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(RejectReason, reject_reason, path=["response"])
    ...
    def test_method_list(self, client: Merge) -> None:
        reject_reason = client.ats.reject_reasons.list()
        assert_matches_type(SyncPage[RejectReason], reject_reason, path=["response"])
...
class TestAsyncRejectReasons:
    ...
    async def test_method_retrieve(self, client: AsyncMerge) -> None:
        reject_reason = await client.ats.reject_reasons.retrieve(
            "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(RejectReason, reject_reason, path=["response"])
    ...
    async def test_method_list(self, client: AsyncMerge) -> None:
        reject_reason = await client.ats.reject_reasons.list()
        assert_matches_type(AsyncPage[RejectReason], reject_reason, path=["response"])
