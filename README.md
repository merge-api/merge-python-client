...
async def main() -> None:
    await client.communication.async_passthrough.create(
        request=DataPassthroughRequest(
            method=MethodEnum.GET,
            path="/scooters",
        ),
...
response = client.communication.async_passthrough.with_raw_response.create(...)
...
```python
client.communication.async_passthrough.create(..., request_options={
    "max_retries": 1
})
```
